...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form method="GET" action="{{ route('admin.kategori-barang.index') }}" class="mb-3">
                ...
                @if ($searchTerm && isset($kategoriBarangList) && $kategoriBarangList->count() > 0 && $statusFilter == 'aktif')
                    <div class="alert alert-info py-2">
                        ...
                        <a href="{{ route('admin.kategori-barang.index') }}" class="btn-link ms-2">Tampilkan Semua
                            Aktif</a>
                    </div>
                @endif
                                        ...
                                        <div class="d-flex gap-1 justify-content-center">
                                            @if ($kategori->trashed())
                                                @can('restore', $kategori)
                                                    <form action="{{ route('admin.kategori-barang.restore', $kategori->id) }}"
    ...
    @can('create', App\Models\KategoriBarang::class)
        @include('admin.kategori.partials.modal-create')
    @endcan
    @if ($kategoriBarangList->first(fn($k) => Gate::allows('update', $k) && !$k->trashed()))
        @include('admin.kategori.partials.modal-edit')
    @endif
            ...
            if (modalTambahKategoriElement) {
                ...
                if (btnTambahKategori) {
                    btnTambahKategori.addEventListener('click', () => {
                        ...
                        if (formTambah) {
                            formTambah.reset();
                            formTambah.action = "{{ route('admin.kategori-barang.store') }}";
                        }
                ...
                button.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            if (formDelete) {
                                formDelete.action =
                                    `{{ route('admin.kategori-barang.destroy', ['kategori_barang' => ':id']) }}`
