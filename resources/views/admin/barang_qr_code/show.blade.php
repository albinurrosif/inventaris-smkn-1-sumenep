...
@section('content')
            ...
            <div class="col-lg-4">
                ...
                <div class="card">
                    ...
                    <div class="card-body d-grid gap-2">
                        ...
                        {{-- Tombol Mutasi (Hanya jika di ruangan & tidak dipinjam/diarsip) --}}
                        @if (
                            $qrCode->id_ruangan &&
                                !$qrCode->id_pemegang_personal &&
                                $qrCode->status !== \App\Models\BarangQrCode::STATUS_DIPINJAM &&
                                (!$qrCode->arsip ||
                                    !in_array($qrCode->arsip->status_arsip, [
                                        \App\Models\ArsipBarang::STATUS_ARSIP_DIAJUKAN,
                                        \App\Models\ArsipBarang::STATUS_ARSIP_DISETUJUI,
                                        \App\Models\ArsipBarang::STATUS_ARSIP_DISETUJUI_PERMANEN,
                                    ])))
                            @can('mutasi', $qrCode)
                                <button type="button" class="btn btn-primary btn-sm btn-mutasi-unit-trigger"
                                    {{-- Class pemicu modal mutasi --}} data-bs-toggle="modal" data-bs-target="#modalMutasiUnit"
                                    data-unit-id="{{ $qrCode->id }}" data-unit-kode="{{ $qrCode->kode_inventaris_sekolah }}"
                                    data-ruangan-asal-id="{{ $qrCode->id_ruangan }}"
                                    data-ruangan-asal-nama="{{ $qrCode->ruangan?->nama_ruangan ?? '' }}"
                                    title="Pindahkan Unit ke Ruangan Lain">
                                    <i class="fas fa-truck me-1"></i> Pindahkan ke Ruangan Lain
                                </button>
                            @endcan
                        @endif

                        {{-- Tombol Serahkan ke Personal (Hanya jika di ruangan, tersedia & tidak dipinjam/diarsip) --}}
                        @if (
                            $qrCode->id_ruangan &&
                                !$qrCode->id_pemegang_personal &&
                                $qrCode->status === \App\Models\BarangQrCode::STATUS_TERSEDIA &&
                            ...
                            @can('assignPersonal', $qrCode)
                                <button type="button" class="btn btn-sm btn-info btn-assign-personal-trigger"
                        ...
                        {{-- Tombol Kembalikan ke Ruangan (Hanya jika dipegang personal & tidak dipinjam/diarsip) --}}
                        @if (
                            $qrCode->id_pemegang_personal &&
                                !$qrCode->id_ruangan &&
                                $qrCode->status !== \App\Models\BarangQrCode::STATUS_DIPINJAM &&
                            ...
                            @can('returnPersonal', $qrCode)
                                <button type="button" class="btn btn-sm btn-success btn-return-personal-trigger"
                                    {{-- CLASS BARU --}} data-bs-toggle="modal" data-bs-target="#modalReturnPersonal"
                                    {{-- TARGET MODAL BARU --}} data-unit-id="{{ $qrCode->id }}"
                                    data-unit-kode="{{ $qrCode->kode_inventaris_sekolah ?? 'N/A' }}"
                        ...
                        {{-- Tombol Transfer Pemegang Personal (Hanya jika dipegang personal & tidak dipinjam/diarsip) --}}
                        @if (
                            $qrCode->id_pemegang_personal &&
                                !$qrCode->id_ruangan &&
                                $qrCode->status !== \App\Models\BarangQrCode::STATUS_DIPINJAM &&
                            ...
                            @can('transferPersonal', $qrCode)
                                <button type="button" class="btn btn-sm btn-primary btn-transfer-personal-trigger"
                                    {{-- CLASS BARU --}} data-bs-toggle="modal" data-bs-target="#modalTransferPersonal"
                                    {{-- TARGET MODAL BARU --}} data-unit-id="{{ $qrCode->id }}"
                                    data-unit-kode="{{ $qrCode->kode_inventaris_sekolah ?? 'N/A' }}"
                        ...
                        {{-- Tombol Arsipkan Unit (Jika tidak dipinjam & belum ada proses arsip aktif/final) --}}
                        @if (
                            $qrCode->status !== \App\Models\BarangQrCode::STATUS_DIPINJAM &&
                                (!$qrCode->arsip ||
                                    !in_array($qrCode->arsip->status_arsip, [
                                            ...
                                            <ul class="list-group list-group-flush">
                                                @foreach ($qrCode->peminjamanDetails->sortByDesc('created_at') as $detailPeminjaman)
                                                    <li class="list-group-item">
                                                        ...
                                                        (Peminjaman ID: <a
                                                            href="{{ route('peminjaman.show', $detailPeminjaman->id_peminjaman) }}">{{ $detailPeminjaman->id_peminjaman }}</a>)
                                                        : Diajukan oleh
    ...
    @include('admin.barang_qr_code.partials.modal_edit_unit', [
    ...
    @include('admin.barang_qr_code.partials.modal_assign_personal', [
    ...
    @include('admin.barang_qr_code.partials.modal_return_personal', [
        'ruangansForReturnForm' => $ruanganListAll,
        'barangQrCodeInstance' => $qrCode,
    ])
    @include('admin.barang_qr_code.partials.modal_transfer_personal', [
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            mutasiUnitTriggerButtons.forEach(button => {
                button.addEventListener('click', function() {
                    if (!modalMutasiInstance) {
                        console.error('Modal Mutasi instance not found');
                        return;
                    }
                    ...
                    form.action =
                        `{{ url('barang-qr-code') }}/${unitId}/mutasi`;
                    if (modalMutasiUnitElement.querySelector('#mutasiUnitId'))
                        modalMutasiUnitElement.querySelector('#mutasiUnitId').value = unitId;
                    ...
                    const suratPemindahan = modalMutasiUnitElement.querySelector(
                        '#mutasiSuratPemindahanPath');
                    if (suratPemindahan) suratPemindahan.value = null;
