...
@section('title', $pageTitle ?? 'Serahkan Unit ke Pemegang Personal')
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">{{ $pageTitle ?? 'Serahkan Unit ke Pemegang Personal' }}</h3>
                    </div>
                    <div class="card-body">
                        @if (session('success'))
                        ...
                        <form action="{{ route('barang-qr-code.assign-personal', $barangQrCode->id) }}" method="POST">
                            @csrf
                            <div class="form-group">
                                <label for="id_pemegang_personal">Pilih Pemegang Personal <span
                                        class="text-danger">*</span></label>
                                <select name="id_pemegang_personal" id="id_pemegang_personal"
                                    class="form-control @error('id_pemegang_personal') is-invalid @enderror" required>
                                    <option value="">-- Pilih Pengguna --</option>
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            {{ old('id_pemegang_personal') == $user->id ? 'selected' : '' }}>
                                            {{ $user->username }} ({{ $user->role }})
                                        </option>
                                    @endforeach
                                </select>
                                @error('id_pemegang_personal')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>

                            <div class="form-group">
                                <label for="catatan_penyerahan_personal">Catatan (Opsional)</label>
                                <textarea name="catatan_penyerahan_personal" id="catatan_penyerahan_personal"
                                    class="form-control @error('catatan_penyerahan_personal') is-invalid @enderror" rows="3">{{ old('catatan_penyerahan_personal') }}</textarea>
                                @error('catatan_penyerahan_personal')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>

                            <div class="d-flex mt-3 justify-content-between">
                                <button type="submit" class="btn btn-primary">Serahkan ke Personal</button>
