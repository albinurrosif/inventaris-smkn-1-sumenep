    ...
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                {{-- Filter Form --}}
                <form method="GET" action="{{ route('admin.ruangan.index') }}" class="mb-3">
                    <div class="row g-2 align-items-end">
                        ...
                        @if ($searchTerm || ($statusFilter != 'aktif' && $statusFilter != null))
                            <div class="col-md-2">
                                <a href="{{ route('admin.ruangan.index') }}" class="btn btn-outline-secondary btn-sm w-100"
                                    title="Reset Filter">
                ...
                @if ($request->search && isset($ruangans) && $ruangans->count() > 0 && $statusFilter == 'aktif')
                    <div class="alert alert-info py-2">
                        ...
                        <a href="{{ route('admin.ruangan.index') }}" class="btn-link ms-2">Tampilkan Semua Aktif</a>
                    </div>
                @endif
                        ...
                        <tbody>
                            @forelse ($ruangans as $index => $ruangan)
                                <tr class="{{ $ruangan->trashed() ? 'table-danger-light' : '' }}">
                                    ...
                                    <td>
                                        @can('view', $ruangan)
                                            <a href="{{ route('admin.ruangan.show', $ruangan->id) }}" class="fw-medium">
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex gap-1 justify-content-center">
                                            @if ($ruangan->trashed())
                                                @can('restore', $ruangan)
                                                    <form action="{{ route('admin.ruangan.restore', $ruangan->id) }}"
            ...
            if (modalTambahRuanganElement) {
                ...
                if (btnTambahRuangan) {
                    btnTambahRuangan.addEventListener('click', () => {
                        ...
                        if (formTambah) {
                            formTambah.reset();
                            formTambah.action = "{{ route('admin.ruangan.store') }}";
                        }
