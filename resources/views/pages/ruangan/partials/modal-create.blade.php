    ...
    <div class="modal-dialog">
        ...
        <form method="POST" action="{{ route('admin.ruangan.store') }}" class="modal-content"
            ...
            <div class="modal-body">
                {{-- Cek apakah ada error di dalam error bag 'storeRuanganErrors' --}}
            @if ($errors->storeRuanganErrors->any())
            <div class="alert alert-danger">
                <h5 class="alert-heading">Gagal Menambahkan Ruangan!</h5>
                <ul class="mb-0">
                    {{-- Loop dan tampilkan setiap pesan error --}}
                    @foreach ($errors->storeRuanganErrors->all() as $error)
                        <li>{{ $error }}</li>
                    @endforeach
                </ul>
            </div>
