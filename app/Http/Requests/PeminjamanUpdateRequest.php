...
class PeminjamanUpdateRequest extends FormRequest
{
    ...
    {
        $user = Auth::user();
        $peminjaman = $this->route('peminjaman'); // Mendapatkan model Peminjaman dari route model binding

        $rules = [
            'tujuan_peminjaman' => 'sometimes|required|string|max:1000',
            'tanggal_rencana_pinjam' => 'sometimes|required|date',
            'tanggal_harus_kembali' => 'sometimes|required|date|after:tanggal_rencana_pinjam',
            'catatan_peminjam' => 'nullable|string|max:1000',
            'id_ruangan_tujuan_peminjaman' => 'nullable|integer|exists:ruangans,id',
        ];

        // Hanya Guru yang boleh mengubah item barang, dan hanya jika status masih 'Menunggu Persetujuan'
